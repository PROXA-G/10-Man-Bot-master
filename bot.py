...
TIME_THRESHOLD = 1.5  # hours cutoff to check previous players when choosing captains
SECS_TO_HOURS = 60 ** 2

class Bot(commands.Bot):
    def __init__(self, command_prefix, drafting_scheme, maps, blacklist, intents):
        """
        Constructor for Bot
            :param command_prefix: symbol to type before command, e.g., "!" for "!<command>"
            :param drafting_scheme: list of strings of format "<A/B><1-2>" where first char is team and 2nd char is # players
            :param maps: list of strings corresponding to map names
        """   
        ...
        self.remove_command("help")  # custom help command
    ...
    async def generate_captains(self, team_a_channel, team_b_channel):
        ...
        potential = []
        check_prev = self.previous_time and (datetime.now() - self.previous_time).seconds / SECS_TO_HOURS <= TIME_THRESHOLD  # seconds to hours conversion
